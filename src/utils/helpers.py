...
def match(catch_merge, ext_merge, date, trashold, window=0):
    ...
    catch_date['key'] = 0
    ext_date['key'] = 0
    result = pd.merge(catch_date, ext_date[['id_vsd','id_fish','fish','volume','unit','date','Region_Plat','key']], on ='key').drop("key", 1)
...
def find_close(result_merge, app_data, num=20):
    ...
    if len(selected_rows) == 0:
        ...
        result['factor'] = (result['catch_volume'].astype(float) / result['volume'].astype(float) - 1).apply(lambda x : abs(x))
